...
describe('Middle specs', () => {
  it('spec 1 sum: any count of arguments', () => {
    function getArgumentsSum(...args) {
      // implementation
    }

    expect(getArgumentsSum(1, 2, 3, 4, 5)).to.eql(15);

    expect(getArgumentsSum()).to.eql(0);
  });
  ...
  it.only('spec 4 read file content', () => {
    const testFilePath = path.resolve(__dirname, 'text.txt');
    ...
    expect(getFileContent(testFilePath)).to.include('Hello World!');
